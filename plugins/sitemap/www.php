...
class www_sitemap extends phpok_plugin
{
	...
	**/
	private function project_all()
	{
		...
		$tplroot = str_replace("_mobile","",$tplroot);
		$tplext = $this->site['tpl_id']['tpl_ext'];
		foreach($rslist as $key=>$value){
			if($value['module'] && !$value['is_front']){
				continue;
			}
	...
	public function web()
	{
		...
		if(!$plist){
			$rslist = array();
			$rslist[0] = array('url'=>$this->config['url'],'lastmod'=>date("Y-m-d",$this->time),'priority'=>'1.0','title'=>$this->site['title']);
		...
		if($total>$this->max_count){
			...
			$listurl = $this->url('plugin','exec','id=sitemap&exec=ginfo&type=web','',true);
			for($i=0;$i<=$page_total;$i++){
				if(strpos($listurl,'?') !== false){
					$url = $listurl.'&pageid='.$i;
				}else{
