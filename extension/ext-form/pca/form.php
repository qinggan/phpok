...
class pca_form extends _init_auto
{
	...
	{
		$this->view($this->dir_extension.'ext-form/pca/config.html','abs-file');
	}
	...
	{
		if(!file_exists($this->dir_extension.'ext-form/pca/cities.xml')){
			$this->error(P_Lang('城市文件不存在'));
		}
		$info = $this->lib('xml')->read($this->dir_extension.'ext-form/pca/cities.xml');
		$this->success($info);
	}

	public function town()
	{
		if(!file_exists($this->dir_extension.'ext-form/pca/districts.xml')){
			$this->error(P_Lang('区县镇文件不存在'));
		}
		$info = $this->lib('xml')->read($this->dir_extension.'ext-form/pca/districts.xml');
		$this->success($info);
	}
	...
	private function _format_admin($rs)
	{
		...
		$province = $this->lib('xml')->read($this->dir_extension.'ext-form/pca/provinces.xml');
		$this->assign('_province',$province['province']);
		$this->assign('_rs',$rs);
		return $this->fetch($this->dir_extension.'ext-form/pca/admin_tpl.html','abs-file');
	}
	...
	private function _format_default($rs)
	{
		...
		$this->assign('_province',$province['province']);
		$this->assign('_rs',$rs);
		return $this->fetch($this->dir_extension.'ext-form/pca/www_tpl.html','abs-file');
	}
}