...
Page({
	...
	onLoad: function (opts) {
		...
		if(!opts || !opts.id){
			if(this.data.identifier){
				opts.id = this.data.identifier;
			}else{
				opts.id = app.okConfig.params.list;
			}
	...
	onPullDownRefresh: function () {
		...
		if(!id){
			id = app.okConfig.params.list;
		}
		this.arclist({'id':id},true,function(){
	...
	onReachBottom: function () {
		...
		var opts = {'id':'product','pageid':next_pageid};
		this.arclist(opts);
	},
