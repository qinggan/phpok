...
;(function($){
	$.admin_upload = function(options){
		...
		this.html = function(rs,i)
		{
			...
			html += '	<input type="button" value="'+p_lang('修改')+'" class="phpok-btn" onclick="obj_'+self.opts.id+'.update(\''+rs.id+'\')" />';
			html += '	<input type="button" value="'+p_lang('预览')+'" class="phpok-btn" onclick="obj_'+self.opts.id+'.preview(\''+rs.id+'\')" />';
			html += '	<input type="button" value="'+p_lang('删除')+'" class="phpok-btn" onclick="obj_'+self.opts.id+'.del(\''+rs.id+'\')" /></div>';
			html += '</div></div>';
			html += '</div>';
			return html;
		};
