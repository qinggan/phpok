...
class ckeditor_control extends phpok_control
{
	...
	**/
	...
	{
		$pageurl = $this->url("ckeditor","arclist");
		...
		//读取所有项目
		$projectlist = $this->model('project')->get_all_project($this->session->val('admin_site_id'));
		$this->assign("projectlist",$projectlist);
		//读取全部列表
		$condition = "l.site_id=".$this->session->val('admin_site_id');
		...
		if($total>0){
			...
			if($total>$psize){
				...
				$string.= '&add='.P_Lang('数量：').'(total)/(psize)'.P_Lang('，').P_Lang('页码：').'(num)/(total_page)&always=0';
